...
function merge_csvs(;example="birkhoff", mode="custom", seeds=1:10, dimensions=3:15, time_limit=1800.0)
    ...
    for dimension in dimensions
        for seed in seeds
            try
                df_temp = DataFrame(CSV.File(joinpath(@__DIR__, "csv/boscia_" * mode * "_" * example * "_" * string(dimension) * "_" * string(seed) * ".csv")))
    ...
    file_name = joinpath(@__DIR__, "csv/boscia_" * mode * "_" * example * ".csv")
...
function build_non_grouped_csv(;option="comparison", example = "birkhoff")
    """
    ...
    file_name = joinpath(@__DIR__, "csv/" * example * "_" * option * "_non_grouped.csv")
    CSV.write(file_name, df, append=false)
    println("\n")
end

function build_grouped_csv(; example="birkhoff", option="comparison", by_time=false)
    ...
    function summarize_by_time(example, timeslots, solver, option)
        ...
        df_ng = DataFrame(CSV.File(joinpath(@__DIR__, "csv/" * example * "_" * option * "_non_grouped.csv")))
    ...
    file_name = joinpath(@__DIR__, "csv/" * example * "_" * option * "_summary_by_" * summary_by_what * ".csv")
    CSV.write(file_name, df, append=false)
    println("\n")
end
...
build_grouped_csv()
build_grouped_csv(by_time=true)
